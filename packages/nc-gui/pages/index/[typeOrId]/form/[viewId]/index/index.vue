...
<template>
  <div class="flex flex-col items-center px-0 sm:px-4 w-full max-w-[720px] pt-0 sm:pt-4 md:pt-6 pb-0 sm:pb-12">
    <div style="box-shadow: 0px 2px 4px hsla(0,0%,8%,.1),0px 6px 12px hsla(0,0%,8%,.06);"
         class="f-form-contianer flex flex-col xs:!shadow-none relative justify-center gap-2 w-full max-w-[max(33%,660px)] m-auto pb-4 pt-30 pb-8 xs:pb-20 rounded-0 px-6 md:px-8 bg-white sm:rounded-lg border-1 border-gray-200">
      <div class="w-full absolute left-0 top-0 h-[280px] z-[0]"
           style="background-image: linear-gradient(rgba(255, 255, 255, 0) 60%, rgba(255, 255, 255, 1));" />
      <template v-if="sharedFormView">
        <h1 class="text-3xl z-1 font-bold self-center text-center mb-4 mt-10">
          {{ sharedFormView.heading }}
        </h1>

        <h2 v-if="sharedFormView.subheading"
            class="prose-lg text-slate-500 text-center dark:text-slate-300 self-center mb-10 leading-6 break-words z-10">
          ...
          <div class="flex justify-center">
            <div v-if="sharedFormView"
              ...
              <div class="pb-12 text-lg flex gap-2 justify-center items-center">
                <component :is="iconMap.circleCheck"
                           class="h-5 w-5 text-green-500" />
                <span>{{ sharedFormView.success_msg || $t('msg.successfullySubmittedFormData') }}</span>
              </div>

              <p v-if="sharedFormView.show_blank_form"
                 class="text-xs text-slate-500 dark:text-slate-300 text-center my-4">
                {{ $t('msg.newFormWillBeLoaded', { seconds: secondsRemain }) }}
              </p>

              <div v-if="sharedFormView.submit_another_form"
                   class="text-center">
                <NcButton class="!px-16 !min-h-11"
                          type="primary"
                          @click="submitted = false"> {{ $t('activity.submitAnotherForm') }}</NcButton>
              </div>
            </div>
          </div>
        </template>
        ...
        <template v-else>
          <a-modal v-model:visible="showCodeScannerOverlay"
                   :closable="false"
                   width="28rem"
                   centered
                   :footer="null"
            ...
            <div class="relative flex flex-col h-full">
              <StreamBarcodeReader v-show="scannerIsReady"
                                   @decode="onDecode"
                                   @loaded="onLoaded"> </StreamBarcodeReader>
            </div>
          </a-modal>
          <GeneralOverlay class="bg-gray-400/75"
          ...
          <div class="nc-form-wrapper">
            <div class="nc-form h-full">
              <div class="flex flex-col gap-6">
                <div v-for="(field, index) in formColumns"
                     :key="index"
                     class="flex flex-col gap-1">
                  <div class="flex nc-form-column-label">
                    <LazySmartsheetHeaderVirtualCell v-if="isVirtualCol(field)"
                  ...
                  <div>
                    <LazySmartsheetDivDataCell class="flex relative min-h-11">
                      <LazySmartsheetVirtualCell v-if="isVirtualCol(field)"
                                                 :model-value="null"
                      ...
                      <a-button v-if="field.enable_scanner"
                        ...
                        <div class="flex items-center gap-1">
                          <component :is="iconMap.qrCodeScan"
                                     class="h-5 w-5" />
                        </div>
                      </a-button>
                    </LazySmartsheetDivDataCell>

                    <div class="flex flex-col gap-2 text-slate-500 dark:text-slate-300 text-[0.75rem] my-2 px-1 leading-[18px]"
                         style="word-break: break-word">
                      <div v-for="error of v$.localState[field.title]?.$errors"
                           :key="error"
                           class="text-red-500">
                        {{ error.$message }}
                      </div>
              ...
              <div class="text-center mt-8">
                <NcButton class="!px-16 !min-h-11 !min-w-50"
                          type="primary"
                          html-type="submit"
...
.f-form-contianer {
  ...
  .nc-form {
    .nc-form-column-label {
      ...
      .name,
      * {
        @apply !text-lg font-bold !pl-0 text-slate-900 !capitalize;
