<script setup lang="ts">
  ...
  const { table, quickImportType, baseTemplate, importData, importColumns, importDataOnly, maxRowsToParse, sourceId, importWorker } =
    defineProps<Props>()
...
<template>
  <a-spin :spinning="isImporting"
          size="large">
    <template #tip>
      <p v-for="(importingTip, idx) of importingTips"
         :key="idx"
         class="mt-[10px]">
        {{ importingTip }}
      </p>
    ...
    <a-card v-if="importDataOnly"
      ...
      <a-form :model="data"
              name="import-only">
        <p v-if="data.tables && quickImportType === 'excel'"
           class="text-center">
      ...
      <a-collapse v-if="data.tables && data.tables.length"
                  v-model:activeKey="expansionPanel"
                  class="template-collapse"
                  accordion>
        <a-collapse-panel v-for="(table, tableIdx) of data.tables"
                          :key="tableIdx">
          <template #header>
          ...
          <a-table v-if="srcDestMapping"
            ...
            <template #headerCell="{ column }">
              ...
              <span v-if="column.key === 'action'">
                <a-checkbox v-model:checked="checkAllRecord[table.table_name]"
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'destination_column'">
                <a-select v-model:value="record.destCn"
                          class="w-52"
                          show-search
                          :filter-option="filterOption"
                          dropdown-class-name="nc-dropdown-filter-field">
                  <a-select-option v-for="(col, i) of columns"
                                   :key="i"
                                   :value="col.title">
      ...
      <a-form ref="formRef"
        ...
        <a-collapse v-if="data.tables && data.tables.length"
          ...
          <a-collapse-panel v-for="(table, tableIdx) of data.tables"
                            :key="tableIdx">
            <template #header>
              <a-form-item v-bind="validateInfos[`tables.${tableIdx}.table_name`]"
                           no-style>
                <div class="flex flex-col w-full">
                  ...
                  <div v-if="formError?.[`tables.${tableIdx}.table_name`]"
            ...
            <a-table v-if="table.columns && table.columns.length"
                     class="template-form"
                     row-class-name="template-form-row"
              ...
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'column_name'">
                  <a-form-item v-bind="validateInfos[`tables.${tableIdx}.columns.${record.key}.${column.key}`]">
                    <a-input :ref="(el: HTMLInputElement) => (inputRefs[record.key] = el)"
                ...
                <template v-else-if="column.key === 'uidt'">
                  <a-form-item v-bind="validateInfos[`tables.${tableIdx}.columns.${record.key}.${column.key}`]">
                    <a-select v-model:value="record.uidt"
                              class="w-52"
                              show-search
                              :filter-option="filterOption"
                              dropdown-class-name="nc-dropdown-template-uidt"
                              @change="handleUIDTChange(record, table)">
                      <a-select-option v-for="(option, i) of uiTypeOptions"
                                       :key="i"
                                       :value="option.value">
                        <a-tooltip placement="right">
                          <template v-if="isSelectDisabled(option.label, table.columns[record.key]?._disableSelect)"
                                    #title>
                            {{
            ...
            <div class="mt-5 flex gap-2 justify-center">
              <a-tooltip bottom>
                ...
                <a-button class="group"
                          @click="addNewColumnRow(tableIdx, 'Number')">
                  <div class="flex items-center">
                    <component :is="iconMap.number"
                               class="group-hover:!text-accent flex text-lg" />
                  </div>
                </a-button>
              </a-tooltip>
              ...
              <a-tooltip bottom>
                ...
                <a-button class="group"
                          @click="addNewColumnRow(tableIdx, 'SingleLineText')">
                  <div class="flex items-center">
                    <component :is="iconMap.text"
                               class="group-hover:!text-accent text-lg" />
                  </div>
                </a-button>
              </a-tooltip>
              ...
              <a-tooltip bottom>
                ...
                <a-button class="group"
                          @click="addNewColumnRow(tableIdx, 'LongText')">
                  <div class="flex items-center">
                    <component :is="iconMap.longText"
                               class="group-hover:!text-accent text-lg" />
                  </div>
                </a-button>
              </a-tooltip>
...
<style scoped lang="scss">.template-collapse {
  @apply bg-white;
}
