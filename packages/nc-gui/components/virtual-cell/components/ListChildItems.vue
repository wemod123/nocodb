...
<template>
  <NcModal v-model:visible="vModel"
    ...
    <LazyVirtualCellComponentsHeader v-if="!isForm"
                                     :relation="relation"
    ...
    <div v-if="!isForm"
         class="flex mt-2 mb-2 items-center gap-2">
      <div class="flex items-center border-1 p-1 rounded-md w-full border-gray-200"
           :class="{ '!border-primary': childrenListPagination.query.length !== 0 || isFocused }">
        <MdiMagnify class="w-5 h-5 ml-2" />
        <a-input ref="filterQueryRef"
    ...
    <div class="flex flex-col flex-grow nc-scrollbar-md pr-1">
      <div v-if="isDataExist || isChildrenLoading"
        ...
        <div class="cursor-pointer pr-1">
          <template v-if="isChildrenLoading">
            <div v-for="(x, i) in Array.from({ length: skeletonCount })"
                 :key="i"
              ...
              <div class="flex flex-col m-[.5rem] gap-2 flex-grow justify-center">
                <a-skeleton-input class="!w-48 !rounded-xl"
                                  active
                                  size="small" />
                <div class="flex flex-row gap-6 w-10/12">
                  <div class="flex flex-col gap-0.5">
                    <a-skeleton-input class="!h-4 !w-12"
                                      active
                                      size="small" />
                    <a-skeleton-input class="!h-4 !w-24"
                                      active
                                      size="small" />
                  </div>
                  <div class="flex flex-col gap-0.5">
      ...
      <div v-else
           class="pt-1 flex flex-col gap-3 my-auto items-center justify-center text-gray-500">
        <InboxIcon class="w-16 h-16 mx-auto" />
        <p>
          {{ $t('msg.noRecordsAreLinkedFromTable') }}
          {{ relatedTableMeta?.title }}
        </p>
        <NcButton v-if="!readonly && childrenListCount < 1"
                  v-e="['c:links:link']"
                  data-testid="nc-child-list-button-link-to"
                  @click="emit('attachRecord')">
          <div class="flex items-center gap-1">
            <MdiPlus /> {{ $t('title.linkMoreRecords') }}
          </div>
        </NcButton>
      </div>
    </div>
    ...
    <div v-if="isMobileMode"
      ...
      <NcPagination v-if="!isNew && childrenList?.pageInfo"
    ...
    <div class="flex flex-row justify-between bg-white relative pt-1">
      <div v-if="!isForm"
           class="flex items-center justify-center px-2 rounded-md text-gray-500 bg-brand-50">
      ...
      <div class="flex flex-row gap-2">
        <NcButton v-if="!isForm"
                  type="ghost"
                  class="nc-close-btn"
                  @click="vModel = false">
          {{ isPublic ? $t('general.close') : $t('general.finish') }}
        </NcButton>
        <NcButton v-if="!readonly && childrenListCount > 0 && !isPublic"
                  v-e="['c:links:link']"
                  data-testid="nc-child-list-button-link-to"
                  @click="emit('attachRecord')">
          <div class="flex items-center gap-1">
