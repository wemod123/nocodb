...
<template>
  <NcTooltip class="z-10"
    ...
    <a-card class="!border-1 group transition-all !rounded-xl relative !mb-2 !border-gray-200 hover:bg-gray-50"
            :class="{
              '!bg-white': isLoading,
      ...
      <div class="flex flex-row items-center justify-start w-full">
        <a-carousel v-if="attachment && attachments && attachments.length"
                    autoplay
                    class="!w-24 !h-24 !max-h-24 !max-w-24">
          <template #customPaging> </template>
          <template v-for="(attachmentObj, index) in attachments">
            <LazyCellAttachmentImage v-if="isImage(attachmentObj.title, attachmentObj.mimetype ?? attachmentObj.type)"
        ...
        <div v-else-if="attachment"
          ...
          <GeneralIcon class="w-full h-full !text-6xl !leading-10 !text-transparent rounded-l-xl"
                       icon="fileImage" />
        </div>
        ...
        <div class="flex flex-col m-[.75rem] gap-1 flex-grow justify-center overflow-hidden">
          <div class="flex justify-between xs:gap-x-2">
            ...
            <MdiLoading v-else-if="isLoading"
                        :class="{
          ...
          <div v-if="fields.length > 0 && !isPublic && !isForm"
               class="flex ml-[-0.25rem] sm:flex-row xs:(flex-col mt-2) gap-4 w-10/12">
            <div v-for="field in fields"
                 :key="field.id"
                 :class="attachment ? 'sm:w-1/3' : 'sm:w-1/4'">
      ...
      <NcButton v-if="!isForm && !isPublic && !readonly"
