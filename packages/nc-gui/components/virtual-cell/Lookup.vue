...
<template>
  <div class="h-full w-full nc-lookup-cell"
       :style="{ height: rowHeight && rowHeight !== 1 ? `${rowHeight * 2}rem` : `2.1rem` }"
       @dblclick="activateShowEditNonEditableFieldWarning">
    <div class="h-full w-full flex gap-1 p-1"
         :class="{
      ...
      <template v-if="lookupColumn">
        ...
        <div v-if="isVirtualCol(lookupColumn)"
          ...
          <template v-if="lookupColumn.uidt !== UITypes.LinkToAnotherRecord ||
            (lookupColumn.uidt === UITypes.LinkToAnotherRecord && lookupColumn.colOptions.type === RelationTypes.BELONGS_TO)
            ">
            <LazySmartsheetVirtualCell v-for="(v, i) of arrValue"
                                       :key="i"
                                       :edit-enabled="false"
                                       :model-value="v"
                                       :column="lookupColumn"
          ...
          <LazySmartsheetVirtualCell v-else
                                     :edit-enabled="false"
                                     :read-only="true"
        ...
        <template v-else>
          <div
            ...
            <LazySmartsheetCell :model-value="arrValue"
                                :column="lookupColumn"
                                :edit-enabled="false"
                                :read-only="true" />
          </div>
          ...
          <template v-else>
            <div class="max-h-full max-w-full w-full nc-cell-lookup-scroll"
                 :class="{
              ...
              <div class="flex gap-1.5 w-full"
                   :class="{
                     ...
                     '!overflow-x-auto nc-scrollbar-x-md pb-1 !overflow-y-hidden':
                       rowHeight === 1 || isAttachment(lookupColumn),
                   }">
                <div v-for="(v, i) of arrValue"
                     :key="i"
                     :class="{
                       'bg-gray-100 px-1 rounded-full min-h-4 max-h-[22px]': !isAttachment(lookupColumn),
                  ...
                  <LazySmartsheetCell :model-value="v"
                                      :column="lookupColumn"
                                      :edit-enabled="false"
                                      :virtual="true"
                                      :read-only="true"
              ...
              <div v-if="showEditNonEditableFieldWarning"
                   class="text-left text-wrap mt-2 text-[#e65100] text-xs">
                {{ $t('msg.info.computedFieldEditWarning') }}
              </div>
              <div v-if="showClearNonEditableFieldWarning"
                   class="text-left text-wrap mt-2 text-[#e65100] text-xs">
