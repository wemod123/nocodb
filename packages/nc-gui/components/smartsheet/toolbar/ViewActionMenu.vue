...
<template>
  <NcMenu class="!min-w-70"
    ...
    <template v-if="!view?.is_default">
      <NcMenuItem v-if="lockType !== LockType.Locked && isUIAllowed('viewCreateOrEdit')"
                  @click="onRenameMenuClick">
        <GeneralIcon icon="edit" />
        {{ $t('activity.renameView') }}
      </NcMenuItem>
      <NcTooltip v-else-if="isUIAllowed('viewCreateOrEdit')">
      ...
      <NcMenuItem v-if="isUIAllowed('viewCreateOrEdit')"
                  @click="onDuplicate">
        <GeneralIcon icon="duplicate"
                     class="nc-view-copy-icon" />
    ...
    <template v-if="view.type !== ViewTypes.FORM">
      ...
      <!-- <NcSubMenu key="upload">
          ...
          <template v-for="(dialog, type) in quickImportDialogs">
            <NcMenuItem v-if="isUIAllowed(`${type}TableImport`) && !isPublicView"
                        :key="type"
                        @click="openQuickImportDialog(type)">
              <div v-e="[
                `a:upload:${type}`,
                {
                  sidebar: props.inSidebar,
        ...
        <template #title>
          <div v-e="[
            ...
            <div class="nc-base-menu-item flex !flex-shrink group !py-1 !px-1 rounded-md bg-brand-50">
              <LazySmartsheetToolbarLockType hide-tick
                                             :type="lockType"
    ...
    <!-- <template v-if="currentBaseId">
      <LazyDlgQuickImport v-for="tp in quickImportDialogTypes"
                          :key="tp"
