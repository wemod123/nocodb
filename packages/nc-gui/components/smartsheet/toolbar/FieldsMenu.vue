...
<template>
  <NcDropdown v-model:visible="open"
    ...
    <div :class="{ 'nc-active-btn': numberOfHiddenFields }">
      <a-button v-e="['c:fields']"
                class="nc-fields-menu-btn nc-toolbar-btn"
                :disabled="isLocked">
        <div class="flex items-center gap-1">
          <GeneralIcon v-if="activeView?.type === ViewTypes.KANBAN || activeView?.type === ViewTypes.GALLERY"
                       icon="creditCard"
                       class="h-4 w-4" />
          ...
          <span v-if="!isMobileMode"
                class="text-capitalize text-sm font-normal">
          ...
          <span v-if="numberOfHiddenFields"
    ...
    <template #overlay>
      <div class="p-4 pr-0 bg-white w-90 rounded nc-table-toolbar-menu"
        ...
        <div class="pr-4"
             @click.stop>
          <a-input v-model:value="filterQuery"
                   :placeholder="$t('placeholder.searchFields')"
                   class="!rounded-lg">
            <template #prefix> <img src="~/assets/nc-icons/search.svg"
                   class="h-3.5 w-3.5 mr-1" /> </template></a-input>
        </div>
