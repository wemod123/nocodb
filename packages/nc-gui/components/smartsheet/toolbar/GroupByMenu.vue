...
<template>
  <NcDropdown v-model:visible="open"
    ...
    <div :class="{ 'nc-active-btn': groupedByColumnIds?.length }">
      <a-button v-e="['c:group-by']"
                class="nc-group-by-menu-btn nc-toolbar-btn"
                :disabled="isLocked">
        <div class="flex items-center gap-1">
          <svg xmlns="http://www.w3.org/2000/svg"
            ...
            <path fill="currentColor"
                  d="M3.58 1.5a1 1 0 0 0-.865.498l-.58 1a1 1 0 0 0 0 1.004l.58 1a1 1 0 0 0 .865.498h1.164a1 1 0 0 0 .865-.498l.58-1a1 1 0 0 0 0-1.004l-.58-1a1 1 0 0 0-.865-.498H3.58Zm0 1h1.164l.58 1l-.58 1H3.58L3 3.5l.58-1Zm3.465 1.998A1 1 0 0 1 7.91 4h1.164a1 1 0 0 1 .865.498l.58 1a1 1 0 0 1 0 1.004l-.58 1A1 1 0 0 1 9.074 8H7.91a1 1 0 0 1-.865-.498l-.58-1a1 1 0 0 1 0-1.004l.58-1ZM9.074 5H7.91l-.58 1l.58 1h1.164l.58-1l-.58-1ZM2.715 6.998A1 1 0 0 1 3.58 6.5h1.164a1 1 0 0 1 .865.498l.58 1a1 1 0 0 1 0 1.004l-.58 1a1 1 0 0 1-.865.498H3.58a1 1 0 0 1-.865-.498l-.58-1a1 1 0 0 1 0-1.004l.58-1Zm2.029.502H3.58L3 8.5l.58 1h1.164l.58-1l-.58-1Z" />
          </svg>

          <!-- Group By -->
          <span v-if="!isMobileMode"
                class="text-capitalize !text-sm font-normal">{{ $t('activity.groupBy') }}</span>

          <span v-if="groupedByColumnIds?.length"
    ...
    <template #overlay>
      <div :class="{ ' min-w-[400px]': _groupBy.length }"
        ...
        <div class="group-by-grid pb-1 mb-2 max-h-100 nc-scrollbar-md pr-5"
             @click.stop>
          <template v-for="[i, group] of Object.entries(_groupBy)"
                    :key="`grouped-by-${group.fk_column_id}`">
            <LazySmartsheetToolbarFieldListAutoCompleteDropdown v-model="group.fk_column_id"
            ...
            <NcSelect ref=""
                      ...
                      @change="saveGroupBy"
                      @click.stop>
              <a-select-option v-for="(option, j) of getSortDirectionOptions(getColumnUidtByID(group.fk_column_id))"
                               :key="j"
                               :value="option.value">
        ...
        <NcButton v-if="fieldsToGroupBy.length > _groupBy.length && _groupBy.length < 3"
                  v-e="['c:group-by:add']"
          ...
          <div class="flex gap-1 items-center"
               :class="{ 'text-brand-500': groupedByColumnIds.length >= _groupBy.length }">
            <div class="flex">
