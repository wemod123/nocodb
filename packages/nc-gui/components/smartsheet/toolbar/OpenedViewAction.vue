...
<template>
  <div v-if="isRenaming"
    ...
    <NcTooltip v-if="error"
               class="absolute top-0.25 right-0.5 bg-white rounded-lg">
      <template #title>
        {{ error }}
      </template>
  ...
  <NcDropdown v-else
    ...
    <div v-e="['c:breadcrumb:view-actions']"
      ...
      <span class="truncate xs:pl-1.25 text-inherit font-normal"
            :class="{
    ...
    <template #overlay>
      <SmartsheetToolbarViewActionMenu :table="activeTable"
                                       :view="activeView"
