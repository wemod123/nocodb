...
<template>
  <div class="flex flex-row border-1 rounded h-8 xs:(h-10 ml-0) ml-1 border-gray-200 overflow-hidden"
    ...
    <div ref="searchDropdown"
      ...
      <GeneralIcon icon="search"
                   class="ml-1 mr-2 h-3.5 w-3.5 text-gray-500 group-hover:text-black" />
      <div v-if="!isMobileMode"
           class="w-16 text-[0.75rem] font-normal text-gray-400 truncate">
        {{ displayColumnLabel }}
      </div>
      ...
      <a-select v-model:value="search.field"
                ...
                @change="onPressEnter">
        <a-select-option v-for="op of columns"
                         :key="op.value"
                         v-e="['c:search:field:select']"
                         :value="op.value">
          <div class="text-[0.75rem] flex items-center -ml-1 gap-2">
            <SmartsheetHeaderIcon class="text-sm"
                                  :column="op.column" />
            {{ op.label }}
          </div>
        </a-select-option>
    ...
    <a-input v-model:value="search.query"
             size="small"
             class="text-xs w-40"
