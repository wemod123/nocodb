...
<template>
  <a-dropdown v-if="!IsPublic"
    ...
    <template #overlay>
      <div v-if="open"
           class="p-6 w-90 bg-white shadow-lg nc-table-toolbar-menu !border-1 border-gray-50"
           @click.stop>
        <div>{{ $t("title.selectFieldToStack") }}</div>
        <div class="nc-fields-list py-2">
          <div class="grouping-field">
            <a-select v-model:value="groupingFieldColumnId"
        ...
        <div class="mt-4 border-1 px-4 pt-4 pb-3 border-gray-50 rounded-2xl">
          <div class="text-base font-normal mb-2">{{ $t("general.options") }}</div>
          <LazySmartsheetColumnEditOrAddProvider v-if="open"
