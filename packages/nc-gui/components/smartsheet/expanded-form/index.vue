<script setup lang="ts">
  ...
  const copyRecordUrl = async () => {
    await copy(
      encodeURI(
        `${dashboardUrl?.value}#/${route.params.typeOrId}/${route.params.baseId}/${meta.value?.id}${props.view ? `/${props.view.title}` : ''
...
<template>
  <NcModal v-if="!isPublic"
           :visible="isExpanded"
           :footer="null"
           :width="commentsDrawer && isUIAllowed('commentList') ? 'min(90vw,1080px)' : 'min(90vw,1080px)'"
    ...
    <div class="h-[88vh] xs:(max-h-full) max-h-215 flex flex-col relative rounded-xl overflow-hidden">
      <div v-if="isLoading"
      ...
      <template v-else>
        <div
             class="flex h-18 p-6 border-b flex-shrink-0 w-full items-center nc-expanded-form-header relative justify-between">
          <template v-if="!isMobileMode">
            <div class="flex gap-3 w-100">
              <div class="flex gap-2">
                <NcButton v-if="props.showNextPrevIcons"
                          :disabled="isFirstRow"
                          type="secondary"
              ...
              <div v-if="isLoading">
                <a-skeleton-input class="!h-8 !sm:mr-14 !w-52 mt-1 !rounded-md !overflow-hidden"
                                  active
                                  size="small" />
              </div>
              <div v-if="row.rowMeta?.new || props.newRecordHeader"
                   class="flex items-center truncate font-bold text-gray-800 text-xl">
                {{ props.newRecordHeader ?? $t('activity.newRecord') }}
              </div>
              <div v-else-if="displayValue && !row.rowMeta?.new"
                   class="flex items-center font-bold text-gray-800 text-xl w-64">
                <span class="truncate">
                  {{ displayValue }}
                </span>
              </div>
            ...
            <div class="flex gap-2">
              <NcButton v-if="false && !isNew && isSuper"
                        type="secondary"
                        class="!xs:hidden text-gray-700"
                        @click="!isNew ? copyRecordUrl() : () => { }">
                <div v-e="['c:row-expand:copy-url']"
                     data-testid="nc-expanded-form-copy-url"
                     class="flex gap-2 items-center">
                  <component :is="iconMap.check"
                             v-if="isRecordLinkCopied"
              ...
              <NcDropdown v-if="!isNew && !isPublic"
                          placement="bottomRight">
                <NcButton type="secondary"
                          class="nc-expand-form-more-actions w-10">
                  <GeneralIcon icon="threeDotVertical"
                               class="text-md text-gray-700" />
                </NcButton>
                <template #overlay>
                  <NcMenu>
                    <NcMenuItem v-if="!isNew"
                                class="text-gray-700"
                                @click="_loadRow()">
                      <div v-e="['c:row-expand:reload']"
                           class="flex gap-2 items-center"
                    ...
                    <NcMenuItem v-if="!isNew && isMobileMode"
                                class="text-gray-700"
                                @click="!isNew ? copyRecordUrl() : () => { }">
                      <div v-e="['c:row-expand:copy-url']"
                           data-testid="nc-expanded-form-copy-url"
                    ...
                    <NcMenuItem v-if="isUIAllowed('dataEdit') && !isNew"
                                class="text-gray-700"
                                @click="!isNew ? onDuplicateRow() : () => { }">
                      <div v-e="['c:row-expand:duplicate']"
                    ...
                    <NcMenuItem v-if="isUIAllowed('dataEdit') && !isNew"
                      ...
                      <div v-e="['c:row-expand:delete']"
                           data-testid="nc-expanded-form-delete"
                           class="flex gap-2 items-center">
                        <component :is="iconMap.delete"
                                   class="cursor-pointer nc-delete-row" />
                        <span class="-ml-0.25">
          ...
          <template v-else>
            <div class="flex flex-row w-full justify-between">
              <NcButton v-if="props.showNextPrevIcons && !isFirstRow"
              ...
              <div v-else
                   class="min-w-10.5"></div>
              <div class="flex justify-center items-center font-semibold text-lg">
                <div>{{ meta.title }}</div>
              </div>
              <NcButton v-if="props.showNextPrevIcons && !islastRow"
        ...
        <div ref="wrapper"
             class="flex flex-grow flex-row w-full"
             style="height:calc(100% - 200px)">
          <div class="flex sm:!w-full xs:w-full flex-col rounded-bl-xl overflow-hidden border-gray-200 xs:(border-0 rounded-none)"
               :class="{
                 'w-2/3': showRightSections,
               }">
            <div
                 class="flex flex-col flex-grow h-full max-h-full nc-scrollbar-md pb-6 items-center w-full bg-white px-6">
              <div v-for="(col, i) of fields"
                ...
                <div class="flex items-start lg:flex-row sm:(gap-x-6) flex-col w-full nc-expanded-cell min-h-10">
                  <div class="w-48 xs:(w-full) mt-0.25 !h-[35px]">
                    <LazySmartsheetHeaderVirtualCell v-if="isVirtualCol(col)"
                                                     class="nc-expanded-cell-header h-full"
                                                     :column="col" />

                    <LazySmartsheetHeaderCell v-else
                  ...
                  <template v-if="isLoading">
                    <div v-if="isMobileMode"
                         class="!h-8.5 !xs:h-12 !xs:bg-white !sm:mr-21 !w-60 mt-0.75 !rounded-lg !overflow-hidden"></div>
                    <a-skeleton-input v-else
                                      class="!h-8.5 !xs:h-9.5 !xs:bg-white !sm:mr-21 !w-60 mt-0.75 !rounded-lg !overflow-hidden"
                                      active
                                      size="small" />
                  </template>
                  <template v-else>
                    <SmartsheetDivDataCell v-if="col.title"
                                           :ref="i ? null : (el: any) => (cellWrapperEl = el)"
                                           class="bg-white rounded-lg !w-[24rem] !xs:w-full border-1 border-gray-200 overflow-hidden px-1 sm:min-h-[35px] xs:min-h-13 flex items-center relative"
                                           :class="{
                                             '!bg-slate-50 !px-0 !select-text': isReadOnlyVirtualCell(col),
                                           }">
              ...
              <div v-if="hiddenFields.length > 0"
                   class="flex w-full sm:px-12 xs:(px-1 mt-2) items-center py-3">
                <div class="flex-grow h-px mr-1 bg-slate-100"></div>
                <NcButton type="secondary"
                          :size="isMobileMode ? 'medium' : 'small'"
                          class="flex-shrink-1 !text-sm"
                          @click="toggleHiddenFields">
                  {{ showHiddenFields ? $t('general.dontShowXHidden', {
                    x: hiddenFields.length, unit: hiddenFields.length
                      > 1 ? `fields` : `field`
                  }) : $t('general.showXHidden', {
                    x: hiddenFields.length, unit:
                      hiddenFields.length > 1 ? `fields` : `field`
                  }) }}
                  <MdiChevronDown class="ml-1"
                                  :class="showHiddenFields ? 'transform rotate-180' : ''" />
                </NcButton>
                <div class="flex-grow h-px ml-1 bg-slate-100"></div>
              </div>
              <div v-if="hiddenFields.length > 0 && showHiddenFields"
                   class="flex flex-col w-full mb-3 items-center">
                <div v-for="(col, i) of hiddenFields"
                     v-show="isFormula(col) || !isVirtualCol(col) || !isNew || isLinksOrLTAR(col)"
                     :key="col.title"
                  ...
                  <div class="sm:gap-x-6 flex sm:flex-row xs:(flex-col) items-start min-h-10">
                    ...
                    <template v-else>
                      <LazySmartsheetDivDataCell v-if="col.title"
                                                 :ref="i ? null : (el: any) => (cellWrapperEl = el)"
                                                 class="!bg-white rounded-lg !w-[24rem] border-1 overflow-hidden border-gray-200 px-1 sm:min-h-[35px] xs:min-h-13 flex items-center relative">
                        <LazySmartsheetVirtualCell v-if="isVirtualCol(col)"
                                                   v-model="_row.row[col.title]"
                                                   :row="_row"
                                                   :column="col"
            ...
            <div v-if="isUIAllowed('dataEdit')"
                 class="w-full h-16 border-t-1 border-gray-200 bg-white flex items-center justify-end p-3 xs:(mt-4 border-t-0 gap-x-4 justify-between)">
              ...
              <div class="flex flex-row gap-x-3">
                ...
                <NcButton v-e="['c:row-expand:save']"
                          data-testid="nc-expanded-form-save"
                          type="primary"
                          size="medium"
                          class="nc-expand-form-save-btn !xs:(text-base)"
          ...
          <div v-if="showRightSections"
               class="nc-comments-drawer relative w-1/3 max-w-125 w-full min-w-90 bg-slate-100 min-w-0 overflow-hidden h-full xs:hidden sm:hidden md:flex"
  ...
  <GeneralDeleteModal v-model:visible="showDeleteRowModal"
                      entity-name="Record"
                      :on-delete="() => onConfirmDeleteRowClick()">
    <template #entity-preview>
      <span>
        <div class="flex flex-row items-center py-2.25 px-2.5 bg-slate-50 rounded-lg text-gray-700 mb-4">
  ...
  <NcModal v-model:visible="preventModalStatus"
           size="small">
    <div class="">
      ...
      <div class="flex flex-row justify-end gap-x-2 mt-5">
        <NcButton type="secondary"
                  @click="discardPreventModal">{{ $t('labels.discard') }}</NcButton>
