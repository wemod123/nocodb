    ...
    try {
      ...
      if ((key && group.column?.uidt === UITypes.Lookup) || group.column?.uidt === UITypes.LinkToAnotherRecord) {
        try {
          ...
          return Array.isArray(k) ? k : [k.toString()]
        } catch {
  ...
  <div class="flex flex-col h-full w-full">
    <div ref="wrapper"
      ...
      <div ref="scrollable"
        ...
        <div v-if="vGroup.root === true"
             class="flex sticky top-0 z-5">
          <div class="bumper mb-2 bg-slate-100"
               style="border-color: #e7e7e9; border-bottom-width: 1px"
               :style="{ 'padding-left': `${(maxDepth || 1) * 13}px` }"></div>
        ...
        <div :class="{ 'px-[12px] pb-4': vGroup.root === true }">
          <a-collapse v-model:activeKey="_activeGroupKeys"
                      class="!bg-transparent w-full nc-group-wrapper"
                      :bordered="false"
                      destroy-inactive-panel
                      @change="findAndLoadSubGroup">
            <a-collapse-panel v-for="[i, grp] of Object.entries(vGroup?.children ?? [])"
                              :key="`group-panel-${i}`"
                  ...
                  <div class="flex items-center">
                    <div class="flex flex-col">
                      <div class="flex gap-2">
                        ...
                        <div class="text-xs text-gray-400 nc-group-row-count">{{ $t('msg.totalXItems', { x: grp.count })
                        }}
                        </div>
                      </div>
                      <div class="flex mt-1">
                        <template v-if="grp.column.uidt === 'MultiSelect'">
                          <span v-for="[tagIndex, tag] of Object.entries(grp.key.split(','))"
                        ...
                        <div v-else-if="!(grp.key in GROUP_BY_VARS.VAR_TITLES) && shouldRenderCell(grp.column)"
                             class="flex min-w-[100px] flex-wrap">
                          <template v-for="(val, ind) of parseKey(grp)"
                                    :key="ind">
                            <GroupByLabel v-if="val"
                                          :column="grp.column"
                                          :model-value="val" />
                            <span v-else
                                  class="text-gray-400">No mapped value</span>
                          </template>
                        </div>
                        <a-tag v-else
              ...
              <GroupByTable v-if="!grp.nested && grp.rows"
                            :group="grp"
    ...
    <LazySmartsheetPagination v-if="vGroup.root"
    ...
    <LazySmartsheetPagination v-else
                              v-model:pagination-data="vGroup.paginationData"
                              align-count-on-right
                              custom-label="groups"
                              show-api-timing
                              ...
                              :fixed-size="fullPage ? props.viewWidth : undefined"></LazySmartsheetPagination>
