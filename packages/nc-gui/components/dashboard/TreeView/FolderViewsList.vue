...
<template>
  <a-menu ref="menuRef"
          :class="{ dragging }"
          class="nc-views-menu flex flex-col w-full !border-r-0 !bg-inherit transition duration-300"
          :selected-keys="selected">
    <DashboardTreeViewCreateViewBtn v-if="isUIAllowed('viewCreateOrEdit')"
                                    class="pl-11"
                                    :align-left-level="isDefaultSource ? 1 : 2">
      <div role="button"
           class="text-slate-400 h-10 hover:text-primary nc-create-view-btn flex flex-row items-center cursor-pointer rounded-md w-full">
    ...
    <template v-if="views.length">
      <DashboardTreeViewFolderViewsNode v-for="view of views"
                                        :id="view.id"
                                        :key="view.id"
                                        :view="view"
                                        :on-validate="validate"
                                        :table="table"
                                        class="nc-view-item !rounded-md !px-0.75 !py-0.5 w-full transition-all ease-in duration-200"
                                        :class="{
