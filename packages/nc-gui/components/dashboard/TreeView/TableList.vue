<script setup lang="ts">
  ...
  const availableTables = computed(() => {
    return tables.value.filter((table) => {
      const matchSource = table.source_id === base.value?.sources?.[sourceIndex.value].id;

      return matchSource && (
        entryConfig.value?.entryToken &&
          entryConfig.value?.scope?.paths &&
          entryConfig.value?.scope?.paths.length > 0
          ? entryConfig.value?.scope?.paths?.some(p => p.endsWith(table.id as string))
          : true
      )
    })
  })
...
<template>
  <div class="border-none sortable-list">
    <template v-if="base">
      ...
      <div v-if="base.sources?.[sourceIndex] && base!.sources[sourceIndex].enabled"
           ref="menuRefs"
           :key="`sortable-${source?.id}-${source?.id && source?.id in keys ? keys[source?.id] : '0'}`"
           :nc-source="source?.id">
        <template v-if="isSuper">
          <TableNode v-for="table of availableTables"
                     :key="table.id"
        ...
        <template v-else>
          <ForderTableNode v-for="table of availableTables"
                           :key="table.id"
                           class="nc-tree-item text-sm"
                           :data-order="table.order"
                           :data-id="table.id"
