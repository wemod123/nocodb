<script setup lang="ts">
  ...
  const columns = [
    {
      title: t('labels.tableEntity'),
      name: 'Table Name',
    },
    {
      title: t('labels.tableViewName'),
      name: 'View Name',
    },
    {
    ...
    <div class="flex flex-col w-[900px]">
      ...
      <div class="flex flex-row items-center w-full mb-4 gap-2 justify-between">
        ...
        <div class="flex">
          <a-button type="text"
                    ghost
                    class="self-start !rounded-md nc-acl-reload"
                    @click="loadTableList">
            <div class="flex items-center gap-2 text-gray-600 font-light">
              <component :is="iconMap.reload"
                         :class="{ 'animate-infinite animate-spin !text-success': isLoading }" />
              {{ $t('general.reload') }}
            </div>
          </a-button>
          ...
          <template #headerCell="{ column }">
            <template v-if="['editor', 'commenter', 'viewer'].includes(column.name)">
              <div class="flex flex-row gap-x-1">
                <NcCheckbox :checked="selectAll[column.name as Role]"
                            @change="() => toggleSelectAll(column.name)" />
            ...
            <div v-if="column.name === 'Table Name'">
              <div class="flex items-center gap-1">
                <div class="min-w-5 flex items-center justify-center">
                  <GeneralTableIcon :meta="{ meta: record.table_meta, type: record.ptype }"
                                    class="text-gray-500" />
                </div>
                <GeneralTruncateText>
              ...
              <div class="flex items-center gap-1">
                <div class="min-w-5 flex items-center justify-center">
                  <GeneralViewIcon :meta="record"
                                   class="text-gray-500"></GeneralViewIcon>
                </div>
            ...
            <div v-for="role in roles"
              ...
              <div v-if="column.name === role">
                <a-tooltip>
                  <template #title>
                    <span> {{ $t(
                      'msg.clickToApplyUIRules', {
                      role: $t(`objects.roleType.${role}`),
                      name: record.title,
                      visible: $t(`msg.${record.disabled[role] ? 'visible' : 'invisible'}`)
                    })
                    }}</span>
                  </template>

                  <NcCheckbox :checked="!record.disabled[role]"
