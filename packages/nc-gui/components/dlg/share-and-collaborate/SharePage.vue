<script lang="ts" setup>
  ...
  async function updateSharedView(setPassword?: string) {
    try {
      if (!activeView.value?.meta) return
      ...
      await $api.dbViewShare.update(activeView.value.id!, {
        meta,
        password: setPassword || activeView.value.password,
      })
    } catch (e: any) {
      message.error(await extractSdkResponseErrorMsg(e))
    }
  ...
  watch(() => passwordProtected.value, (n, o) => {
    if (n === true && o === false) {
      setTimeout(async () => {
  ...
  <div class="flex flex-col py-2 px-3 mb-1">
    <div class="flex flex-col w-full bg-white mt-2.5 px-3 py-2.5 border-gray-200 border-1 rounded-md gap-y-2">
      <div class="flex flex-row w-full justify-between py-0.5">
        <div class="flex"
             :style="{ fontWeight: 500 }">{{ $t('activity.enabledPublicViewing') }}</div>
        <a-switch v-e="['c:share:view:enable:toggle']"
      ...
      <template v-if="isPublicShared">
        ...
        <div class="flex flex-col justify-between mt-1 py-2 px-3 bg-gray-50 rounded-md">
          <div class="flex flex-row justify-between">
            ...
            <a-switch v-e="['c:share:view:password:toggle']"
        ...
        <div class="flex flex-col justify-between gap-y-3 mt-1 py-2 px-3 bg-gray-50 rounded-md">
          <div v-if="activeView &&
            ...
            <a-switch v-model:checked="allowCSVDownload"
          ...
          <div v-if="activeView?.type === ViewTypes.FORM"
            ...
            <div class="flex flex-row justify-between">
              ...
              <a-switch v-e="['c:share:view:theme:toggle']"
            ...
            <Transition name="layout"
              ...
              <div v-if="viewTheme"
                   class="flex -ml-1">
                <LazyGeneralColorPicker data-testid="nc-modal-share-view__theme-picker"
