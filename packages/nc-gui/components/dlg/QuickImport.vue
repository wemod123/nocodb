<script setup lang="ts">
  ...
  const { importType, importDataOnly = false, table, sourceId, ...rest } = defineProps<Props>()
...
<template>
  <a-modal v-model:visible="dialogShow"
    ...
    <a-spin :spinning="isParsingData"
            :tip="progressMsg"
            size="large">
      <div class="px-2">
        <div class="prose-xl font-weight-bold -mt-1">{{ importMeta.header }}</div>

        <div class="mt-2 px-2 overflow-y-auto nc-scrollbar-md py-4"
             style="max-height:calc(100vh - 240px)">
          <LazyTemplateEditor v-if="templateEditorModal"
          ...
          <a-tabs v-else
            ...
            <a-tab-pane key="uploadTab"
              ...
              <div class="py-6">
                <a-upload-dragger v-model:fileList="importState.fileList"
                                  name="file"
            ...
            <a-tab-pane v-if="isImportTypeJson"
                        key="jsonEditorTab"
                        :closable="false">
              <template #tab>
              ...
              <div class="pb-3 pt-3">
                <LazyMonacoEditor ref="jsonEditorRef"
                                  v-model="importState.jsonEditor"
            ...
            <a-tab-pane v-else
              ...
              <div class="pr-10 pt-5">
                <a-form :model="importState"
                        name="quick-import-url-form"
                        layout="vertical"
                        class="mb-0">
                  <a-form-item :label="importMeta.urlInputLabel"
                               v-bind="validateInfos.url">
                    <a-input v-model:value="importState.url"
                             size="large" />
                  </a-form-item>
                </a-form>
              </div>
        ...
        <div v-if="!templateEditorModal">
          ...
          <div class="mb-4">
            ...
            <a-form-item class="!my-2"
                         :label="t('msg.info.footMsg')"
                         v-bind="validateInfos.maxRowsToParse">
              <a-input-number v-model:value="importState.parserConfig.maxRowsToParse"
                              :min="1"
                              :max="50000" />
            </a-form-item>

            <a-form-item v-if="!importDataOnly"
                         class="!my-2">
            ...
            <a-form-item v-if="isImportTypeCsv || IsImportTypeExcel"
                         class="!my-2">
            ...
            <!-- Flatten nested -->
            <a-form-item v-if="isImportTypeJson"
                         class="!my-2">
            ...
            <a-form-item v-if="!importDataOnly"
                         class="!my-2">
              <a-checkbox v-model:checked="importState.parserConfig.shouldImportData">{{ $t('labels.importData') }}
              </a-checkbox>
            </a-form-item>
          </div>
        </div>
    ...
    <template #footer>
      <a-button v-if="templateEditorModal"
                key="back"
                class="!rounded-md"
                @click="templateEditorModal = false">{{ $t('general.back') }}
      </a-button>

      <a-button v-else
                key="cancel"
                class="!rounded-md"
                @click="dialogShow = false">{{ $t('general.cancel') }} </a-button>

      <a-button v-if="activeKey === 'jsonEditorTab' && !templateEditorModal"
                key="format"
                class="!rounded-md"
...
<style lang="pcss">
  .nc-modal-quick-import.ant-modal-wrap{
