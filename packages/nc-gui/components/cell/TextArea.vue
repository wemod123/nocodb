<script setup lang="ts">
  ...
  const height = computed(() => {
    if (!rowHeight.value || rowHeight.value === 1) return 26

    return rowHeight.value * 26
  })
...
<template>
  <textarea v-if="isForm"
            :ref="focus"
            v-model="vModel"
            rows="4"
            class="h-full w-full outline-none border-none !p-0"
            :style="{
              minHeight: `${height}px`,
              resize: 'none'
            }"
  ...
  <NcDropdown v-else
    ...
    <div class="flex flex-row w-full"
      ...
      <div v-if="isRichMode"
           ...
           @dblclick="onExpand">
        <LazyCellRichText v-model:value="vModel"
                          sync-value-change
                          readonly
                          class="!pointer-events-none" />
      </div>
      <textarea v-else-if="editEnabled && !isVisible"
                :ref="focus"
                v-model="vModel"
                rows="4"
      ...
      <span v-else-if="vModel === null && showNull"
            class="nc-null uppercase">{{ $t('general.null') }}</span>

      <LazyCellClampedText v-else-if="rowHeight"
                           :value="vModel"
                           :lines="rowHeight"
      ...
      <NcTooltip v-if="!isVisible"
                 placement="bottom"
                 :class="{ '!bottom-2 !right-2': editEnabled && !isVisible && !isRichMode }"
                 class="!absolute right-0 bottom-0 !hidden nc-text-area-expand-btn">
        ...
        <NcButton type="secondary"
          ...
          <component :is="iconMap.expand"
                     class="transform group-hover:(!text-grey-800 ) scale-120 text-gray-700 text-xs" />
        </NcButton>
      </NcTooltip>
    </div>
    ...
    <template #overlay>
      <div v-if="isVisible"
        ...
        <LazyCellRichText v-else-if="isVisible"
                          v-model:value="vModel"
                          show-menu
                          full-mode
                          :read-only="readOnly" />
